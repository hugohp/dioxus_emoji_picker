...
#[component]
fn CancelIcon(picker_status: Signal<PickerStatus>, search_text: Signal<String>) -> Element {
    // <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
...
fn EmojiSearch(
    ...
    let skin_tone_emoji = use_memo(move || options().skin_tone_emoji);

    let skin_tone_icon = use_memo(move || {
        let current_skin_tone = *SKIN_TONE.read();
        skin_tone_emoji()
            .with_skin_tone(current_skin_tone)
            .unwrap()
            .as_str()
    });
...
#[component]
fn EmojiCategory(picker_status: Signal<PickerStatus>) -> Element {
    match &*picker_status.read() {
...
#[component]
fn EmojiGroups(picker_status: Signal<PickerStatus>) -> Element {
    let groups = vec![
